const movieInfo = (function () {
  ...
  function renderTheatres() {
    ...
    // Filter theaters to allocate only two theaters
    const allocatedTheatres = screeningsformovie.theatresOnDate.slice(0, 2);
  
    if (allocatedTheatres.length === 0) {
      theatreListElement.innerHTML = '<div class="no-shows">No Shows on Date. Please select another date.</div>';
      return;
    }
  
    allocatedTheatres.forEach(theatreId => {
      const thisTheatre = dataServer.getTheatre(theatreId);
      let screeningsList = '';
  
      // Filter showtimes for the selected theaters
      ...
      // Only render theaters with available showtimes
      if (screeningsList.length > 0) {
        theatreListElement.innerHTML += `<div class="theatre">
          ...
          <div class="theatre-timings-div">
            ...
            <div class="theatre-timings-list">${screeningsList}</div>
          </div>
        </div>`;
      }
    });
  ...
  // Call the renderTheatres function to display the allocated theaters.
  renderTheatres();
